...
const appwriteConfig = {
  ...
  messageCollcetionId:"679e564b003e1cc0e384",
};
...
export async function getMessagesOnly() {
  try {
    const response = await databases.listDocuments(
      appwriteConfig.databaseId,
      appwriteConfig.messageCollcetionId
    );
    return response.documents; 
  } catch (error) {
    console.error("Error fetching announcements:", error.message);
...
export async function getUsername(userId) {
  try {
    console.log('szukam user for userId:', userId); 
...
export async function getAvatar(userId) {
  try {
    console.log('szukam avatar for userId:', userId); 
    ...
    if (userDocuments.documents.length > 0) {
      const userDocument = userDocuments.documents[0];
      console.log("URL obrazu:", userDocument.avatar);  
...
export async function getEmail(userId) {
  try {
    ...
    throw new Error(error.message || "Failed to get email");
  }
}
export async function getPhonenumber(userId) {
  try {
    ...
    throw new Error(error.message || "Failed to get phone number");
  }
}
