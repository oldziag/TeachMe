...
const appwriteConfig = {
  ...
  zajeciaCollectionId:"67a26103002510189ce4",
  relationsCollectionId:"67a3780c000e1ee1f75d",
};
...
export async function createAd(title, description, category,level, price, userId) {
  try {
...
export async function getAds() {
  try {
    ...
    console.log("Ads fetched:", response.documents); // Log the documents
...
export async function createEvent(title, userId, day, hour, studentId) {
  try {
 
    const newEvent = await databases.createDocument(
      appwriteConfig.databaseId,
      appwriteConfig.zajeciaCollectionId,
      ID.unique(),
      {
        title,
        userId: userId,
...
export async function getEvents() {
  try {
    ...
    console.log("Events fetched:", response.documents);
    return response.documents; 
  } catch (error) {
...
export const setAvatar = async (userId, link) => {
  try {
    ...
    console.error("Błąd podczas zmiany awatara:", err);
    throw err;
  }
}
export async function getRelationships() {
  try {
    const response = await databases.listDocuments(
      appwriteConfig.databaseId,
      appwriteConfig.relationsCollectionId,
    );
    console.log("Relations fetched:", response.documents);
    return response.documents; 
  } catch (error) {
    console.error("Error fetching relations:", error.message);
    throw new Error("Relations could not be loaded");
  }
}